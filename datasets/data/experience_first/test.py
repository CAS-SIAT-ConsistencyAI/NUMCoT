...
from run_medium import score as score_function

def split_and_sort_number_regex(number):
    number_str = str(number)
    parts = re.findall(r'.{1,4}', number_str[::-1])
    parts = [part[::-1] for part in parts]

    parts.sort(reverse=True)  # 对部分进行排序
            ...
            for index, row in df.iterrows():
                ...
                for col_name, element in row.items():
                    if "出现问题，需要重跑" in str(element):
                        ...
                        problem_details[var_name].append((index + 1, col_name, row['number'], row['answer']))
    ...
    for file, count in sorted(counts.items(), key=lambda item: item[1], reverse=True):
        ...
        if file in problem_details:
            for row_index, col_name, number_value, answer in problem_details[file]:
                ...
                print(f"    - Problem at Row {row_index}, Column '{col_name}', Number: {number_value}, Response: {response}, score: {score}")

print(format_quesion('en', 'word2num', 'fraction', 'XXX', shot='zeroshot'))
print(format_quesion('en', 'word2num', 'fraction', 'XXX', shot='cot_zeroshot'))
print(format_quesion('en', 'word2num', 'fraction', 'XXX', shot='fewshot'))
print(format_quesion('en', 'word2num', 'fraction', 'XXX', shot='cot_fewshot'))
